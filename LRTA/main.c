...
void adicionaAlcancaveis(int matriz[20][35], Lista* listaAberta, Lista* listaFechada, Lista* listaPontos){
    if(personX + 1 < 19 && matriz[personX + 1][personY] != 'X' && verificaIgual(listaAberta, personX + 1, personY) == 0 && verificaIgual(listaFechada, personX + 1, personY) == 0){
        auxiliar = consulta_Ponto(listaPontos, personX + 1, personY);
        //auxiliar.custoF = calculaHeuristica(personX+1, personY) + matriz[personX][personY];
        auxiliar.custoF = auxiliar.heuristica + matriz[personX][personY];
        //auxiliar.posicaoX = personX + 1;
        //auxiliar.posicaoY = personY;
        insere_lista_ordenada(listaAberta, auxiliar);
        //remove_lista(listaPontos, personX + 1, personY);
    }
    ...
    if(personY + 1 < 35 && matriz[personX][personY + 1] != 'X' && verificaIgual(listaAberta, personX, personY + 1) == 0 && verificaIgual(listaFechada, personX, personY + 1) == 0){
        auxiliar = consulta_Ponto(listaPontos, personX, personY + 1);
        ...
        insere_lista_ordenada(listaAberta, auxiliar);
        //remove_lista(listaPontos, personX, personY + 1);
    }
...
void inicializaHeuristica(int matriz[20][35], int i, int j, Lista* listaPontos){
    for(i = 18; i > 0;i--){
        for(j = 33; j > 0;j--){
            if(matriz[i][j] != 'X'){
                auxiliar.heuristica = calculaHeuristica(i,j);
                auxiliar.custoF = 0;
                auxiliar.posicaoX = i;
                auxiliar.posicaoY = j;
                insere_lista_ordenada(listaPontos, auxiliar);
            }
        }
    }
}

void lookAhead(int matriz[20][35], Lista* listaAberta, Lista* listaFechada){
    menorCusto = consulta_lista(listaAberta);
    auxiliar = consulta_Ponto(listaFechada, personX, personY);
    if(auxiliar.heuristica < menorCusto.custoF){
        auxiliar.heuristica = menorCusto.custoF;
...
int main(){
    Lista* listaPontos = cria_lista();
    Lista* listaFechada = cria_lista();
    Lista* listaAberta = cria_lista();

    srand(time(NULL));

    int mat[20][35], i = 0, j = 0; //Cria a matriz que ira ser o cenário
    ...
    //Inicializa o cenario, adicionando custo 1 a todos os locais que não tem obstaculo

    inicializaCenario(mat, i, j);

    //Inicializa a heurística

    inicializaHeuristica(mat, i, j, listaPontos);
    ...
    while(!fim && !lista_vazia(listaAberta)){
        ...
        adicionaAlcancaveis(mat, listaAberta, listaFechada, listaPontos);

        //Verifica o custo F do ponto adiante

        lookAhead(mat, listaAberta, listaFechada);
